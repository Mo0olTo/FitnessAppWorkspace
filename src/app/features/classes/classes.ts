...
import { MealsStore } from '../../store/meals/meals.store';


@Component({
  selector: 'app-classes',
  imports: [SectionTitle, FilterTabs, ReuseableCarousel, WorkoutDetails],
...
export class Classes implements OnInit {
  ...
  classLevels = FILTER_TABS.classLevels;
  ...
  exerciseTabs=computed<any[]>(()=>[
    FULL_BODY_CARDS,
    ...this.exercises().map(m=>({
      _id:m._id , 
      name:m.exercise
    }))
  ])
  ...
  canGoToStep2 = computed(() => {
    return !!this.exercisefacade.selectedMuscleId() &&
           !!this.exercisefacade.selectedDifficultyId();
  });
  ...
  onMuscleCardClick(muscleId: string): void {
    ...
    this.exercisefacade.setDifficulty(
      '69d982ed85f6bfa972bf2216'
    );
